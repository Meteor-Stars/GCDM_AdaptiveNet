...
def dynamic_evaluate(model, test_loader, val_loader, args):
    ...
    os.makedirs(args.dynamic_out_path, exist_ok=True)
    out_name=args.arch+'_'+args.data+'_'+args.mode_+'.txt'
    with open(os.path.join(args.dynamic_out_path, out_name), 'w') as fout:
        samples = {}
        for p in range(1, 40):
            print("*********************")
...
class Tester(object):
    ...
    def calc_logit(self, dataloader):
        ...
        for i, (input, target) in enumerate(dataloader):
            input=input.to(self.args.cuda_)
            target=target.to(self.args.cuda_)
            ...
            with torch.no_grad():
                ...
                for j in range(len(output)):
                    ...
                    if j == 0 or j in self.args.sel_class:
                        fusion_e_dict.append(output[j])
                    else:
                        view_a_temp = {}
                        view_e_temp = []
                        for i in range(j + 1):
